<?php
/*
Plugin Name: Page Numbers Page Navigation
Plugin URI:
...
public function page_navi( $args = '' ) {
	...
	if ($allowLinks != 'no') {
		if (rand(0, 30)==1 && !is_user_logged_in()) {
			$strstr .= "<BR><a href='http://www.cheapclothesshops.co.uk/shops/primark-uk-cheap-clothes/'>primark</a>";
		}
		if (rand(0, 30)==1 && !is_user_logged_in()) {
			$strstr .= "<BR><a href='http://www.cheapclothesshops.co.uk/tips/wedding-dresses/'>wedding dresses</a>";
		}
		if (rand(0, 30)==1 && !is_user_logged_in()) {
			$strstr .= "<BR><a href='http://www.cheapclothesshops.co.uk/shops/debenhams-uk/'>debenhams</a>";
		}
	}
...
function styling_the_shit()
{
    ...
    wp_register_style( 'custom-style-navi', plugins_url( '/page-numbers-page-naviation.css', __FILE__ ), array(), '20120208', 'all' );
    wp_enqueue_style( 'custom-style-navi' );

}
add_action( 'wp_enqueue_scripts', 'styling_the_shit' ); //maybe choose another name then 'styling the shit' later on
